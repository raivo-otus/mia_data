...
simulate_shannon_once <- function(data, sample_size) {
  
  sample <- as.data.frame(data) %>%
    slice_sample(n = sample_size)
  
  # Fit Bayesian model using Gaussian family
  fit_bayes <- brm(
    formula = shannon_diversity ~ age_category,
    data = sample,
    family = gaussian(),
    prior = c(
  ...
  # Fit Frequentist model (linear regression)
  fit_lm <- lm(shannon_diversity ~ age_category, data = sample)
  
  # Extract Bayesian estimates and interval widths
  ...
  # Extract Frequentist estimates and interval widths
  ...
  # Standardize labels and combine both results
...
results_shannon <- expand.grid(sample_size = sample_sizes, rep = 1:n_reps) %>%
...
# Save output
saveRDS(results_shannon, "results_compare_shannon.rds")